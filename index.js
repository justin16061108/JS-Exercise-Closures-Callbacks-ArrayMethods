...
function getFullNames(runners) {
  ...
  return runnerNames.push(`${name.last_name}, ${name.first_name}`)
  });
  return runnerNames;
}
...
function firstNamesAllCaps(runners) {
  return runners.map(firstName => firstName.first_name.toUpperCase());
}
...
function tallyUpDonations(runners) {
  return runners.reduce((accumulater, tally) => accumulater + tally.donation, 0)
}
